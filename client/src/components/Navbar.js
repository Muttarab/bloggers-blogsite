...
const Navbar = () => {
  ...
  const imgbefore = `http://localhost:${process.env.PORT}/${userprofiledata.slice(7,)}`;
  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get(`http://localhost:${process.env.PORT}/userprofile/${user.id}`);
